...
namespace bit_math{
    ...
    bitset<2048> Power(bitset<2048> a, bitset<2048> b, bitset<2048> p){
...
string Find_Primary(bitset<2048> p, bitset<2048> q, bitset<2048> e){
    bitset<2048> temp(0);
    bitset<2048> flag;
    flag = Exten_GCD(e, phi_Euler(p, q), 1, 0, temp);
    if (bit_math::Subtract(flag, 1).none() != 1){
        return "-1";
    }
    if (temp.test(1023) & 1 == 1){
        temp = bit_math::Addition(temp, phi_Euler(p, q));
    }
...
void Task(char** argv){
    ...
    bitset<2048> p(myText[0]), q(myText[1]), e(myText[2]);
    string result = Find_Primary(p, q, e);
